...
import adminauth_support

def vp_start_gui():
    '''Starting point when module is the main routine.'''
    global val, w, root
    root = Tk()
    top = Admin_Authentication___Health_App (root)
    adminauth_support.init(root, top)
    root.mainloop()

w = None
def create_Admin_Authentication___Health_App(root, *args, **kwargs):
    '''Starting point when module is imported by another program.'''
    global w, w_win, rt
    rt = root
    w = Toplevel (root)
    top = Admin_Authentication___Health_App (w)
    adminauth_support.init(w, top, *args, **kwargs)
    return (w, top)

def destroy_Admin_Authentication___Health_App():
    global w
    w.destroy()
    w = None


class Admin_Authentication___Health_App:
    def __init__(self, top=None):
        ...
        top.title("Admin Authentication : Health App")
        ...
        self.Label3.place(relx=0.267, rely=0.275, height=29, width=241)
        ...
        self.Label3.configure(text='''Admin Authentication''')
        ...
        self.Text1_3.place(relx=0.457, rely=0.606, relheight=0.062, relwidth=0.408)
        ...
        def login():
            self.input1 = str(self.Text1.get("1.0",'end-1c')).strip()
            #self.input2 = str(self.Text1_3.get("1.0",'end-1c')).strip()
            self.input2 = str(self.Text1_3.get())
            if self.input1 == "admin" and self.input2 == "admin":
                root.destroy()
                subprocess.call("python2 admindashscreen1.py")




        self.Button1 = Button(self.Frame2,command=login)
        ...
        self._img1 = ImageTk.PhotoImage(PIL.Image.open("./login.png"))
